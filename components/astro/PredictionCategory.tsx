    ...
    <Card>
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xl sm:text-2xl font-bold text-primary-600">{category.score}</span>
        </div>
        <Progress value={category.score} color={getProgressColor(category.score)} showLabel />
      </CardHeader>
      <CardContent className="space-y-3 sm:space-y-4">
        <p className="text-gray-700 leading-relaxed text-sm sm:text-base">{category.description}</p>
        
        {category.positiveAspects.length > 0 && (
          <div>
            <h4 className="font-semibold text-green-700 mb-2">Positive Aspects</h4>
